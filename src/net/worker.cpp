...
/* Attempt to close a connection. If the SQ is full, defer for retry. */
static void try_close(int fd, Connection **conns, IoOps *ops,
    ...
    if (ret < 0 && state->pending_close_count < MAX_PENDING_CLOSE)
...
static void handle_accept(const IoCompletion *comp, Connection **conns,
    ...
    if (!conn) {
        ...
        try_close(fd, conns, ops, backend, state);
        return;
    }
    conns[fd] = conn;
    ops->submit_nodelay(backend, fd); /* non-critical, ignore failure */
...
static void handle_recv(const IoCompletion *comp, Connection **conns,
    ...
    /* Response buffer accumulates all pipelined replies at increasing offsets.
     * One coalesced send is issued after the loop — avoids N SQEs per pipeline
     * batch and prevents buffer overwrite corruption when multiple commands
     * share the same stack buffer. */
    ...
    while (offset < parse_len) {
        ...
        if (result == RespParseResult::OK) {
            uint32_t cap = RESPONSE_BUF_SIZE - resp_offset;
            uint32_t resp_len = command_execute(&cmd, store, response + resp_offset, cap);
            resp_offset += resp_len;
            offset += consumed;

            if (resp_offset > RESPONSE_BUF_SIZE - 4096) {
                if (ops->submit_send(backend, fd, response, resp_offset) < 0) {
                    try_close(fd, conns, ops, backend, state);
                    goto recycle;
                }
                resp_offset = 0;
    ...
    while (config->running->load(std::memory_order_relaxed)) {
        ...
        if (wstate.pending_close_count > 0) {
            ...
            for (int i = 0; i < wstate.pending_close_count; i++) {
                ...
                if (config->ops.submit_close(config->backend, fd) < 0)
        ...
        for (int i = 0; i < n; i++) {
            ...
            switch (c->kind) {
            case IoCompletion::ACCEPT: handle_accept(c, conns, &config->ops, config->backend, listen_fd, &wstate); break;
            case IoCompletion::RECV:   handle_recv(c, conns, &store, &config->ops, config->backend, &wstate); break;
            case IoCompletion::SEND:   handle_send(c, conns, &config->ops, config->backend, &wstate); break;
            case IoCompletion::CLOSE:  handle_close(c, conns, &config->ops, config->backend, &wstate); break;
            case IoCompletion::ERROR:  handle_error(c, conns, &config->ops, config->backend, &wstate); break;
            case IoCompletion::IGNORE: break;
            }
        }
    }
