...
static inline int decode_fd(uint64_t ud) { return static_cast<int>(static_cast<uint32_t>(ud)); }

static constexpr int BUF_GROUP_ID = 0;

[[maybe_unused]] static inline bool is_power_of_two_u32(uint32_t v) { return v != 0 && (v & (v - 1u)) == 0; }

[[maybe_unused]] static inline bool completion_kind_valid(IoCompletion::Kind kind) {
    return kind == IoCompletion::ACCEPT || kind == IoCompletion::RECV || kind == IoCompletion::SEND ||
           kind == IoCompletion::CLOSE || kind == IoCompletion::ERROR || kind == IoCompletion::IGNORE;
}
...
static int uring_init(IoBackend* ctx) {
    ...
    params.flags = IORING_SETUP_COOP_TASKRUN | IORING_SETUP_DEFER_TASKRUN | IORING_SETUP_SINGLE_ISSUER |
    ...
    ret = posix_memalign(&ring_mem, page_size, sizeof(struct io_uring_buf_ring) + ring_entries_size);
    if (ret != 0) {
    ...
    for (uint32_t i = 0; i < be->buf_count; i++) {
        io_uring_buf_ring_add(be->buf_ring, be->buf_pool + (i * be->buf_size), be->buf_size,
...
static int uring_submit_nodelay(IoBackend* ctx, int fd) {
    ...
    if (!sqe)
        return -ENOSPC;
    io_uring_prep_cmd_sock(sqe, SOCKET_URING_OP_SETSOCKOPT, fd, IPPROTO_TCP, TCP_NODELAY,
...
static void uring_recycle_buffer(IoBackend* ctx, uint16_t buf_id) {
    ...
    int mask = io_uring_buf_ring_mask(be->buf_count);
    io_uring_buf_ring_add(be->buf_ring, be->buf_pool + (static_cast<uint32_t>(buf_id) * be->buf_size),
...
static void uring_recycle_buffers(IoBackend* ctx, const uint16_t* buf_ids, uint32_t count) {
    ...
    for (uint32_t i = 0; i < count; i++) {
        ...
        if (static_cast<uint32_t>(buf_id) >= be->buf_count)
            continue;
        io_uring_buf_ring_add(be->buf_ring, be->buf_pool + (static_cast<uint32_t>(buf_id) * be->buf_size),
...
static void uring_destroy(IoBackend* ctx) {
    ...
    auto* be = reinterpret_cast<UringBackend*>(ctx);
    if (be->ring_initialized)
        io_uring_queue_exit(&be->ring);
    if (be->buf_ring)
        std::free(be->buf_ring);
    if (be->buf_pool)
        std::free(be->buf_pool);
    std::free(be);
}
