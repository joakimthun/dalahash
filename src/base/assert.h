...
#define ASSERT_FMT(condition, fmt, ...)                                                          \
    do {                                                                                         \
        if (!(condition))                                                                        \
            dalahash_assert_fail_fmt(#condition, __FILE__, __LINE__, __func__, (fmt)            \
                                     __VA_OPT__(, ) __VA_ARGS__);                                \
    } while (0)
#else
// Compiled out in non-debug builds.
#define ASSERT(condition, message) do { (void)sizeof(condition); } while (0)
#define ASSERT_FMT(condition, fmt, ...) do { (void)sizeof(condition); } while (0)
#endif

#if defined(DALAHASH_DEBUG_ASSERTS)
// Mark impossible control-flow in debug and crash with context if reached.
#define DALAHASH_UNREACHABLE(message) \
    do { dalahash_assert_fail("unreachable", __FILE__, __LINE__, __func__, (message)); } while (0)
#elif defined(__GNUC__) || defined(__clang__)
#define DALAHASH_UNREACHABLE(message) do { __builtin_unreachable(); } while (0)
#else
#define DALAHASH_UNREACHABLE(message) do { std::abort(); } while (0)
#endif
