...
static int sim_submit_accept(IoBackend* ctx, int) {
    ...
    if (be->submit_accept_fail_count > 0) {
        be->submit_accept_fail_count--;
        return -ENOSPC;
    }
...
static int sim_submit_recv(IoBackend* ctx, int fd) {
    auto* be = reinterpret_cast<SimIoBackend*>(ctx);
    if (be->submit_recv_fail_count > 0) {
...
static int sim_submit_send(IoBackend* ctx, int fd, const uint8_t* data, uint32_t len) {
    auto* be = reinterpret_cast<SimIoBackend*>(ctx);
    if (be->submit_send_fail_count > 0) {
...
static inline IoOps sim_io_ops() {
    return IoOps{
        .init = sim_init,
        .submit_accept = sim_submit_accept,
        .submit_recv = sim_submit_recv,
        .submit_send = sim_submit_send,
        .submit_close = sim_submit_close,
        .submit_nodelay = sim_submit_nodelay,
        .recycle_buffer = sim_recycle_buffer,
        .recycle_buffers = sim_recycle_buffers,
        .wait = sim_wait,
        .destroy = sim_destroy,
    };
}
...
static inline IoCompletion sim_recv(SimIoBackend* be, int fd, const void* data, uint32_t len) {
    be->buffers.emplace_back(reinterpret_cast<const uint8_t*>(data),
                             reinterpret_cast<const uint8_t*>(data) + len);
    uint16_t buf_id = static_cast<uint16_t>(be->buffers.size() - 1);
    // more=true: multishot stays armed — worker won't try to resubmit.
    return {.kind = IoCompletion::RECV,
            .fd = fd,
            .result = static_cast<int>(len),
            .buf = be->buffers.back().data(),
            .buf_len = len,
            .buf_id = buf_id,
            .more = true};
}
...
static inline IoCompletion sim_close(int fd) {
    return {.kind = IoCompletion::CLOSE,
            .fd = fd,
            .result = 0,
            .buf = nullptr,
            .buf_len = 0,
