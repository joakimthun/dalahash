...
TEST(SharedKvEviction, ConcurrentPressureProducesEvictionsAndStaysStable) {
    ...
    for (uint32_t w = 0; w < kWriters; w++) {
        threads.emplace_back([&, w]() {
            ...
            while (!stop.load(std::memory_order_acquire)) {
                ...
                if ((seq & 127u) == 0) kv_store_quiescent(s, w);
            }
        ...
        threads.emplace_back([&, r]() {
            ...
            while (!stop.load(std::memory_order_acquire)) {
                ...
                KvGetStatus gs = kv_store_get(
                    s, worker, keys[idx], logical_now.load(std::memory_order_acquire), &out);
    ...
    EXPECT_TRUE(kvtest::converge_to_capacity(
        s, 0, logical_now.load(std::memory_order_acquire), 2000));
    ...
    for (const std::string &k : keys) {
        KvValueView out = {};
        if (kv_store_get(s, 0, k, final_now, &out) == KvGetStatus::HIT) hits++;
    }
...
TEST(SharedKvConcurrency, DisjointWritersMonotonicReaders) {
    ...
    for (uint32_t i = 0; i < kWriterCount; i++) keys.push_back("owner:" + std::to_string(i));
    ...
    for (uint32_t w = 0; w < kWriterCount; w++) {
        threads.emplace_back([&, w]() {
            ...
            for (uint32_t i = 1; i <= kIters; i++) {
                ...
                if ((i & 255u) == 0) kv_store_quiescent(s, w);
            }
            kv_store_quiescent(s, w);
        });
    }
        ...
        threads.emplace_back([&, r]() {
            ...
            while (!failed.load(std::memory_order_acquire)) {
                ...
                for (uint64_t v : last_seen) {
                    if (v < kIters / 4u) { all_done = false; break; }
                }
                if (all_done) break;
            }
            kv_store_quiescent(s, worker);
...
TEST(SharedKvTTLConcurrency, WritersWithShortTTLEventuallyExpireAfterStop) {
    ...
    KvStore *s = kv_store_create(&cfg);
    ASSERT_NE(s, nullptr);
    for (uint32_t i = 0; i < kWorkers; i++) ASSERT_EQ(kv_store_register_worker(s, i), 0);
    ...
    for (uint32_t i = 0; i < kWorkers; i++) kv_store_quiescent(s, i);
    ...
    for (uint32_t r = 0; r < kReaders; r++) {
        threads.emplace_back([&, r]() {
            ...
            while (!stop.load(std::memory_order_acquire)) {
                KvValueView out = {};
                KvGetStatus gs = kv_store_get(
                    s, worker, "shared:key", logical_now.load(std::memory_order_acquire), &out);
                if (gs == KvGetStatus::HIT) {
                    uint32_t tid = 0;
                    uint64_t seq = 0, salt = 0;
                    if (!kvtest::parse_value(kvtest::view_to_string(out), &tid, &seq, &salt))
    ...
    for (uint32_t i = 0; i < kWorkers; i++) {
        threads.emplace_back([&, i]() {
            ...
            for (uint32_t iter = 0; iter < 25000; iter++) {
                ...
                if ((iter & 63u) == 0) kv_store_quiescent(s, i);
            }
